      ...
      if thumbSize != "Default":
        ...
        if thumbSize == 'To Box':
          ...
          if sourceAspectRatio == destAspectRatio:
            thumb = thumb.scaled(w,h, mode = QtCore.Qt.SmoothTransformation)
          else:
            # If the destination box does not have the same aspect ratio as the source, we need to add a letterbox
            scaledThumb = thumb.scaledToHeight(h, mode = QtCore.Qt.SmoothTransformation)
          ...
          thumb = thumb.scaledToWidth(w, mode = QtCore.Qt.SmoothTransformation)
        elif thumbSize == 'Scaled to Height':
          thumb = thumb.scaledToHeight(h, mode = QtCore.Qt.SmoothTransformation)
