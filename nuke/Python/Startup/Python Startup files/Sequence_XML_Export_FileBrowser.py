# Custom Item Wrapper copied from the following git for testing purposes:
# https://github.com/antiero/dotStudio/blob/master/Python/Startup/quick_export.py
...
class ExportersMenu:
  ...
  def __init__(self):
    ...
    self.rootMenu = QtGui.QMenu("Quick Export XML...")
    # Populate
    self.populateQuickExportMenus()

    # Should also add this to the Timeline and Spreadsheet
  ...
  def populateQuickExportMenus(self):
    ...
    for preset in self.presets:
      ...
      if 'XML' in preset.name():
        if isinstance(preset,hiero.exporters.FnTimelineProcessor.TimelineProcessorPreset):
          self.rootMenu.addAction(act)
