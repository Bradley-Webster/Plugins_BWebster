...
def fromHDR2084(x):
  ...
  return sign * 100.0 * (L ** 6.277394636)
...
for i in xrange(NUM_SAMPLES):
  x = i/(NUM_SAMPLES-1.0)
  x = Fit(x, 0.0, 1.0, RANGE[0], RANGE[1])
  data.append(fromHDR2084(x))

WriteSPI1D('luts/hdr2084.spi1d', RANGE[0], RANGE[1], data)

cs = OCIO.ColorSpace(name='HDR2084')
cs.setDescription("HDR2084")
cs.setBitDepth(OCIO.Constants.BIT_DEPTH_F32)
cs.setAllocation(OCIO.Constants.ALLOCATION_UNIFORM)
cs.setAllocationVars([RANGE[0], RANGE[1]])

t = OCIO.FileTransform('hdr2084.spi1d', interpolation=OCIO.Constants.INTERP_LINEAR)
cs.setTransform(t, OCIO.Constants.COLORSPACE_DIR_TO_REFERENCE)
config.addColorSpace(cs)
