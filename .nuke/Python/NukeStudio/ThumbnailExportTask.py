        ...
        if thumbSize != "Default":
          ...
          if thumbSize == 'To Box':
            ...
            if sourceAspectRatio == destAspectRatio:
              thumb = thumb.scaled(w,h, mode = QtCore.SmoothTransformation)
            else:
              # If the destination box does not have the same aspect ratio as the source, we need to add a letterbox
              scaledThumb = thumb.scaledToHeight(h, mode = QtCore.SmoothTransformation)
            ...
            thumb = thumb.scaledToWidth(w, mode = QtCore.SmoothTransformation)
          elif thumbSize == 'Scaled to Height':
            thumb = thumb.scaledToHeight(h, mode = QtCore.SmoothTransformation)
